...
class AccountingReportService
{
    ...
    /**
     * Build AR Aging buckets as of a date. Returns array with bucket totals.
     * Buckets: current, 1-30, 31-60, 61-90, 90+
     */
    public function buildReceivablesAging(?string $asOf = null): array
    {
        if (!config('accounting.features.ar_ap') || !Schema::hasTable('customer_invoices')) {
            return [
                'as_of' => $asOf ?: now()->toDateString(),
                'buckets' => ['current' => 0.0, '30' => 0.0, '60' => 0.0, '90' => 0.0, '120+' => 0.0],
                'count' => 0,
            ];
        }

        $asOfDate = $asOf ? Carbon::parse($asOf)->endOfDay() : Carbon::now()->endOfDay();
        $invoices = DB::table('customer_invoices')
            ->leftJoin('customer_receipts', 'customer_invoices.id', '=', 'customer_receipts.customer_invoice_id')
            ->select('customer_invoices.id', 'customer_invoices.due_date', 'customer_invoices.amount', DB::raw('COALESCE(SUM(customer_receipts.amount),0) as received'))
            ->whereDate('customer_invoices.date', '<=', $asOfDate->toDateString())
            ->groupBy('customer_invoices.id', 'customer_invoices.due_date', 'customer_invoices.amount')
            ->get();

        $buckets = ['current' => 0.0, '30' => 0.0, '60' => 0.0, '90' => 0.0, '120+' => 0.0];
        $count = 0;
        foreach ($invoices as $inv) {
            $outstanding = max(0.0, (float)$inv->amount - (float)$inv->received);
            if ($outstanding <= 0) { continue; }
            $count++;
            $due = $inv->due_date ? Carbon::parse($inv->due_date) : $asOfDate;
            $days = $due->diffInDays($asOfDate, false); // negative if not due yet
            if ($days <= 0) {
        ...
        return ['as_of' => $asOfDate->toDateString(), 'buckets' => $buckets, 'count' => $count];
    }

    /**
     * Build AP Aging buckets as of a date. Returns array with bucket totals.
     * Buckets: current, 1-30, 31-60, 61-90, 90+
     */
    public function buildPayablesAging(?string $asOf = null): array
    {
        if (!config('accounting.features.ar_ap') || !Schema::hasTable('vendor_bills')) {
            return [
                'as_of' => $asOf ?: now()->toDateString(),
                'buckets' => ['current' => 0.0, '30' => 0.0, '60' => 0.0, '90' => 0.0, '120+' => 0.0],
                'count' => 0,
        ...
        $bills = DB::table('vendor_bills')
            ->leftJoin('vendor_payments', 'vendor_bills.id', '=', 'vendor_payments.vendor_bill_id')
            ->select('vendor_bills.id', 'vendor_bills.due_date', 'vendor_bills.amount', DB::raw('COALESCE(SUM(vendor_payments.amount),0) as paid'))
            ->whereDate('vendor_bills.date', '<=', $asOfDate->toDateString())
            ->groupBy('vendor_bills.id', 'vendor_bills.due_date', 'vendor_bills.amount')
            ->get();

        $buckets = ['current' => 0.0, '30' => 0.0, '60' => 0.0, '90' => 0.0, '120+' => 0.0];
        $count = 0;
        foreach ($bills as $b) {
            $outstanding = max(0.0, (float)$b->amount - (float)$b->paid);
            if ($outstanding <= 0) { continue; }
            $count++;
            $due = $b->due_date ? Carbon::parse($b->due_date) : $asOfDate;
            $days = $due->diffInDays($asOfDate, false);
            if ($days <= 0) {
                $buckets['current'] += $outstanding;
            } elseif ($days <= 30) {
                $buckets['30'] += $outstanding;
