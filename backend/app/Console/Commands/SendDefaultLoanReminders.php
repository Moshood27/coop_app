...
class SendDefaultLoanReminders extends Command
{
    ...
    public function handle(): int
    {
        ...
        \App\Models\QardHasan::whereIn('status', ['active', 'defaulted'])
            ->whereNull('defaulted_at')
            ->whereColumn('paid_amount', '<', 'principal_amount')
            ->chunkById(100, function ($activeLoans) use ($dry, &$countFlagged) {
                foreach ($activeLoans as $loan) {
                    if ($loan->getOverdueDays() >= 7) { // 7 days threshold
                        ...
                        if (!$dry) {
                            ...
                            $this->info("[DR] Would flag loan {$loan->qard_id_string} as defaulted.");
                        }
                    }
                }
            });
        ...
        \App\Models\QardHasan::whereIn('status', ['active', 'defaulted'])
            ...
            ->chunkById(100, function ($defaultedLoans) use ($dry) {
        ...
        User::query()
            ...
            ->chunkById(100, function ($users) use ($dry, &$countUsers, &$countEmails) {
                foreach ($users as $user) {
                    ...
                    $countUsers++;
                    if ($dry) {
                        $this->info(sprintf('[DR] Would send to %s <%s> | Loans: %d | Outstanding: %.2f', $user->full_name, $user->email, count($loansData), $totalOutstanding));
                        continue;
                    }
    ...
    protected function syncMissingPenalties(bool $dry): void
    {
        ...
        \App\Models\QardHasan::whereNotNull('defaulted_at')->chunkById(100, function ($defaultedLoans) use ($dry) {
            foreach ($defaultedLoans as $loan) {
                ...
                if (!$exists) {
                    if (!$dry) {
                        ...
                        $this->info("[DR] Would create missing penalty record for loan {$loan->qard_id_string}");
                    }
                }
            }
        });
        ...
        \App\Models\LoanPenalty::whereNull('default_cleared_at')->chunkById(100, function ($activePenalties) use ($dry) {
